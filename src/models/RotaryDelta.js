...
(function(exports) {
    ...
    RotaryDelta.prototype.homeAngle = function() {
        var that = this;
        return (that.homeAngles.theta1 + that.homeAngles.theta2 + that.homeAngles.theta3) / 3;
    }
    RotaryDelta.prototype.zBowlError = function(pulsesCenter, pulsesRadius, eTheta) {
        var that = this;
    ...
    RotaryDelta.prototype.degreePulses = function() {
        var that = this;
        return that.steps360 * that.microsteps * that.gearRatio / 360;
    }
    RotaryDelta.prototype.calcXYZ = function(angles) {
        var that = this;
        ...
        var d = b * b - 4.0 * a * c;
        if (d < 0) { // point exists
            that.verbose && console.log("RotaryDelta calcXYZ(", angles, ") point exists");
            return null;
        }
    ...
    RotaryDelta.prototype.calcPulses = function(xyz) {
        var that = this;
    ...
    RotaryDelta.prototype.calcAngles = function(xyz) {
        var that = this;
        var x = xyz.x;
        var y = xyz.y;
        var z = xyz.z - that.dz;
        ...
        if (theta1 == null) {
            that.verbose && console.log("calcAngles(", xyz, ") theta1 is null");
            return null;
        }
        var theta2 = that.calcAngleYZ(x * cos120 + y * sin120, y * cos120 - x * sin120, z);
        if (theta2 == null) {
            that.verbose && console.log("calcAngles(", xyz, ") theta2 is null");
            return null;
        }
        var theta3 = that.calcAngleYZ(x * cos120 - y * sin120, y * cos120 + x * sin120, z);
        if (theta3 == null) {
            that.verbose && console.log("calcAngles(", xyz, ") theta3 is null");
            return null;
        }
        return {
    ...
    ///////////// CLASS ////////////
    RotaryDelta.createLooseCanonRAMPS = function() {
...
(typeof describe === 'function') && describe("RotaryDelta", function() {
    ...
    it("has home angle option", function() {
        shouldEqualT(new RotaryDelta().homeAngles, {
            theta1: -67.2,
            theta2: -67.2,
            theta3: -67.2
        });
    ...
    it("has home pulses option that overrides home angle option", function() {
        shouldEqualT(new RotaryDelta().homePulses, {
            p1: -11200,
            p2: -11200,
            p3: -11200
        });
    ...
    it("has steps per revolution option", function() {
        new RotaryDelta().steps360.should.equal(400);
        new RotaryDelta({
            steps360: 200
        }).steps360.should.equal(200);
    });
    it("has microsteps option", function() {
        new RotaryDelta().microsteps.should.equal(16);
        new RotaryDelta({
            microsteps: 8
        }).microsteps.should.equal(8);
    });
    it("has pulley gear ratio option", function() {
        new RotaryDelta().gearRatio.should.equal(150 / 16);
        new RotaryDelta({
            gearRatio: 100 / 16
    ...
    it("has homing error options", function() {
        new RotaryDelta().eTheta1.should.equal(0);
        new RotaryDelta({
            eTheta1: 3.1
        }).eTheta1.should.equal(3.1);
        new RotaryDelta().eTheta2.should.equal(0);
        new RotaryDelta({
            eTheta2: 3.1
        }).eTheta2.should.equal(3.1);
        new RotaryDelta().eTheta3.should.equal(0);
        new RotaryDelta({
            eTheta3: 3.1
    ...
    it("homeAngle() should return average home angle", function() {
        var e = 0.0001;
        RotaryDelta.createLooseCanonRAMPS().homeAngle().should
            .within(60.33 - e, 60.33 + e);
    });
    it("calcXYZ() should compute XYZ from angles ", function() {
        var dc = new RotaryDelta();
    ...
    it("xyz(0,0,0) should be at theta(0,0,0)", function() {
        var dc = new exports.RotaryDelta();
        shouldEqualT(dc.calcXYZ({
            theta1: 0,
            theta2: 0,
            theta3: 0
    ...
    it("homing error should affect Y and Z", function() {
        ...
        var dc_minus1 = new RotaryDelta({
            eTheta1: -1,
            eTheta2: -1,
            eTheta3: -1
        });
        var dc_plus1 = new RotaryDelta({
            eTheta1: 1,
            eTheta2: 1,
            eTheta3: 1
        });
    ...
    it("gradient", function() {
        ...
        console.log("p\tz\tdx\tdy\tdz\tdz_x50\tzbowl");
    ...
    it("should process hex probe data", function() {
        ...
        for (var eTheta = -15; eTheta <= 15; eTheta++) {
            ...
            console.log("eTheta:", eTheta, " zErr:", zError);
        }
        ...
        //console.log("deg0Avg:", deg0Avg);
        //var deg60Avg = (data[2]+data[5])/2;
        //console.log("deg60Avg:", deg60Avg);
        //var deg120Avg = (data[3]+data[6])/2;
        //console.log("deg120Avg:", deg120Avg);
    });
    it("should match C++ FireStep kinematics", function() {
        var delta = RotaryDelta.createLooseCanonRAMPS();
